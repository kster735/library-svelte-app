<script>
  ...
  $: ids = pg.substring(1).split('/');  // location.href.split("#")[1].substring(1).split('/');

  $: bg = ($activeTheme === 'light' ? 'light' : 'dark' );
</script>

<nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb" class="bread container rounded shadow-sm text-capitalize" data-bs-theme={bg}>
